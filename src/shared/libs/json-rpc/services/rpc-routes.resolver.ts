...
import { JsonRpcExplorer } from './explorer.service';
...
export const RPC_MAPPING_MESSAGE = (
  name: string,
  path: string,
  method: string,
...
export class RpcRoutesResolver implements Resolver {
  private readonly logger = new Logger(RpcRoutesResolver.name);
  ...
  public registerRouters(
    ...
    return routes.map((rpcMethodHandler) => {
      ...
      this.logger.log(RPC_MAPPING_MESSAGE(rpcHandlerName, path, rpcMethodName));
